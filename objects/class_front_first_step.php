...
class cleanto_first_step
{
	...
	*/
	...
	function get_already_booked_slots($selected_date,$cur_time_interval,$service_padding_before,$service_padding_after,$booking_padding_time,$staff_id){
		...
		$return_arr = array();
		if ($staff_id == '1') {
			$query="select DISTINCT `booking_date_time` from `ct_bookings` where booking_date_time LIKE '%".$use_date."%' AND `staff_ids` = '' and (`booking_status`='A' OR `booking_status`='C') ";
			$value=mysqli_query($this->conn,$query);
		}else{
			$query="select DISTINCT `booking_date_time` from `ct_bookings` where booking_date_time LIKE '%".$use_date."%' AND FIND_IN_SET(".$staff_id.",`staff_ids`) and (`booking_status`='A' OR `booking_status`='C') ";
			$value=mysqli_query($this->conn,$query);
		}
 ...
 $query="SELECT `start_date_time`,`end_date_time` FROM `".$this->table_name4."` where provider_id = '".$staff_id."'" ;
    ...
    /* A new function for new design */
    function get_day_time_slot_by_provider_id($provider_schedule_type, $start_date, $time_int,$advance_bookingtime=0,$service_padding_before=0,$service_padding_after=0,$timezonediff=0,$booking_padding_time=0,$staff_id=1)    
    {
  ...
  //function to get duration of each booking added by geek_sonu
	function get_booking_time_duration($start_date, $staff_id)
	{
	 $return_arr = array();
	 		if ($staff_id == '1') {
	 			$query="SELECT `order_id`,`booking_date_time` FROM `ct_bookings` where `booking_date_time` LIKE '%".$start_date."%' AND `staff_ids` = ''";
	 		}else{
	 			$query="SELECT `order_id`,`booking_date_time` FROM `ct_bookings` where `booking_date_time` LIKE '%".$start_date."%' AND FIND_IN_SET(".$staff_id.",`staff_ids`)";
	 		}
		$result=mysqli_query($this->conn,$query);	
		// print_r($result);
		$counter = 0;
